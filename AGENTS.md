...
Admins edit page hero/banner images and social links at `/admin/site-content` (`pages/admin/ManageSiteContent.tsx`) — this is the ONLY social-links editor (the Settings page social tab was removed). Storage: one `siteImages` setting (group `appearance`, map of image key → Cloudinary URL) + individual `*_url` settings in the `social` group. Public reads go through `GET /api/settings/public` (no auth, returns only `siteImages` + `social` group); the full `GET /api/settings` now requires admin auth. Client merge: `context/SiteContentContext.tsx` (`useSiteContent()` → `{ images, social }`, falls back to `IMAGES`/`SOCIAL` defaults on any failure; blank override = default). All public pages/Footer/Navbar/JsonLd/GuestCard consume the context — never reintroduce direct `IMAGES`/`SOCIAL` imports in live components.
